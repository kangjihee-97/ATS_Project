...
public class ApplicationController {
    ...
    @PostMapping("/write")
    public String write(
            @ModelAttribute ApplicationVO vo,
    ...
    public void downloadResume(@PathVariable int id,
        ...
        String filename = URLEncoder.encode(
            app.getResumeOriginalName(), StandardCharsets.UTF_8).replace("+", "%20");
    ...
    public void downloadExcel(
            @RequestParam(defaultValue = "applicationId") String sort,
        ...
        headerStyle.setFillForegroundColor(
            new XSSFColor(new byte[]{(byte)92,(byte)42,(byte)14},null));
        headerStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        XSSFFont hf = wb.createFont(); hf.setBold(true);
        hf.setColor(IndexedColors.WHITE.getIndex()); hf.setFontHeightInPoints((short)11);
        ...
        XSSFCellStyle left    = makeDataStyle(wb,false); left.setAlignment(HorizontalAlignment.LEFT);
        XSSFCellStyle leftAlt = makeDataStyle(wb,true);  leftAlt.setAlignment(HorizontalAlignment.LEFT);

        String today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy년 MM월 dd일"));
        Row titleRow = sh.createRow(0); titleRow.setHeightInPoints(28);
        ...
        Row hr = sh.createRow(2); hr.setHeightInPoints(22);
        ...
        for(ApplicationVO app:list){
            Row row=sh.createRow(rowNum); row.setHeightInPoints(20);
        ...
        XSSFCellStyle totalStyle=wb.createCellStyle(); totalStyle.cloneStyleFrom(headerStyle);
        totalStyle.setFillForegroundColor(new XSSFColor(new byte[]{(byte)139,(byte)69,(byte)19},null));
        Row totalRow=sh.createRow(rowNum); totalRow.setHeightInPoints(20);
        Cell tot=totalRow.createCell(0); tot.setCellValue("총 "+list.size()+"명");
        ...
        String filename=URLEncoder.encode(
            "ATS_지원자목록_"+LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"))+".xlsx",
    ...
    private void sortList(List<ApplicationVO> list, String sort, String dir) {
        Comparator<ApplicationVO> cmp = switch(sort) {
            case "applicantName" -> Comparator.comparing(ApplicationVO::getApplicantName, Comparator.nullsLast(String::compareTo));
            case "postingTitle"  -> Comparator.comparing(ApplicationVO::getPostingTitle,  Comparator.nullsLast(String::compareTo));
    ...
    private XSSFCellStyle makeDataStyle(XSSFWorkbook wb,boolean alt){
        ...
        s.setFillForegroundColor(alt
            ?new XSSFColor(new byte[]{(byte)253,(byte)248,(byte)243},null)
    ...
    private String stageLabel(String s){
        if(s==null)return"-";
        return switch(s){case"RECEIVED"->"서류접수";case"DOC_PASS"->"서류합격";
            case"INTERVIEW1"->"1차 면접";case"INTERVIEW2"->"2차 면접";
            case"FINAL"->"최종합격";case"FAIL"->"불합격";default->s;};
    }
    private String sortLabel(String sort,String dir){
        String col=switch(sort){case"applicantName"->"지원자명";case"postingTitle"->"공고명";
            case"email"->"이메일";case"careerYear"->"경력";
            case"stage"->"단계";case"appliedAt"->"지원일";default->"번호";};
